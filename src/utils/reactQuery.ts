...
export function generateUseMutationHook<T extends MutationAsyncFunction>(
  ...
  }) as MutationFnWithoutVariables & MutationFnWithVariables
...
export function generateUseQueryHook<T extends AsyncFunction>(fn: T, queryKey: string | string[]) {
  ...
  type Params = MultiParams[0]
  type Options = Omit<
    UseQueryOptions<ResponseType, AxiosError, ResponseType>,
    'queryFn' | 'queryKey'
  >
  ...
  // Variant 2: With only options
  type QueryFnWithoutVariables = (
    options?: Options,
  ) => ReturnType<typeof useQuery<ResponseType, AxiosError>>

  const queryFn = ((
    ...args: Params extends undefined ? [Options?] : [...MultiParams, Options?]
  ) => {
